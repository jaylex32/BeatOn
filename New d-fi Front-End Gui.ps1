...
function URL  {
    ...
    {   
        ...
        [Console]::OutputEncoding = [Text.UTF8Encoding]::UTF8; .\d-fi.exe /c -q "$QQualityFiles" -b -o "$SpotMusicpath/$PlaylistName/$SpotLayoutMusic" -d -i $Playlistpath | Out-Host
        ...
        Remove-Item "$Trackpath" -ErrorAction SilentlyContinue
    ...
    elseif ($DeezerUrlRadiobutton.IsChecked)
    {
        ...
        {
         $deezerjobs = & {[Console]::OutputEncoding = [Text.UTF8Encoding]::UTF8; .\d-fi.exe /c -u "$($Albumsindex.Where({$_.Albums -eq $Albums_Listbox.SelectedItem}).QobuzUrl)" -q "$DQualityFiles" -o "$DMusicpath/$DLayoutMusic" -d | Out-Host } & Start-Sleep -Milliseconds 2000
         $Search_Box.Clear()
         ...
         $PlaylistJobs = & { [Console]::OutputEncoding = [Text.UTF8Encoding]::UTF8; .\d-fi.exe /c -q "$DQualityFiles" -o "$DMusicpath/$DLayoutMusic" -d -i $Playlistpath | Out-Host } & Start-Sleep -Milliseconds 2000
         Remove-Item "$Playlistpath" -ErrorAction SilentlyContinue
         $Search_Box.Clear()
         ...
         $Qobuzjobs = & { [Console]::OutputEncoding = [Text.UTF8Encoding]::UTF8; .\d-fi.exe /c -u "$albumsresults" -q "$QQualityFiles" -d -b -o "$QMusicpath/$QLayoutMusic" | Out-Host } & Start-Sleep -Milliseconds 2000
         $Search_Box.Clear()
