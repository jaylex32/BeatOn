...
<Window x:Name="d_fi_Front_End1" x:Class="WpfApp1.MainWindow"
    ...
    <Grid Width="900" Margin="0,0,0,-16" Height="600">
        ...
        <RadioButton x:Name="ArtistRadiobutton" GroupName="DownloadOptions" Content="Artist" HorizontalAlignment="Left" Margin="720,250,0,0" VerticalAlignment="Top" Foreground="#FFEDEDED" Width="52" FontWeight="Bold" FontFamily="Constantia" Height="18" RenderTransformOrigin="0.5,0.5"/>
        <RadioButton x:Name="SongsRadiobutton" GroupName="DownloadOptions" Content="Songs" HorizontalAlignment="Left" Margin="720,298,0,0" VerticalAlignment="Top" FontWeight="Bold" Foreground="White" Width="52" Height="18" FontFamily="Constantia"/>
        <RadioButton x:Name="AlbumsRadiobutton" GroupName="DownloadOptions" Content="Albums" HorizontalAlignment="Left" Margin="720,346,0,0" VerticalAlignment="Top" FontWeight="Bold" Foreground="White" Width="63" Height="18" FontFamily="Constantia"/>
        <RadioButton x:Name="PlaylistRadiobutton" GroupName="DownloadOptions" Content="Playlist" HorizontalAlignment="Left" Margin="720,394,0,0" VerticalAlignment="Top" FontWeight="Bold" Foreground="White" Width="63" Height="18" FontFamily="Constantia"/>
        <RadioButton x:Name="FeaturedRadiobutton" GroupName="DownloadOptions" Content="Featured" HorizontalAlignment="Left" Margin="720,442,0,0" VerticalAlignment="Top" FontWeight="Bold" Foreground="White" Width="69" Height="18" FontFamily="Constantia"/>
        ...
        <Border x:Name="Albums_Covers_Border"
    ...
    <Image x:Name="Albums_Covers"
           Source="https://creazilla-store.fra1.digitaloceanspaces.com/cliparts/3486515/album-cover-with-notes-01-clipart-md.png"
           Stretch="Uniform"
           HorizontalAlignment="Left"
        ...
        <Label x:Name="Poster_Label" HorizontalAlignment="Left" Margin="37,405,0,0" VerticalAlignment="Top" Width="138" Foreground="White">
         <Label.Content>
    ...
    <StackPanel Orientation="Horizontal">
        ...
        <Image Source="https://github.com/jaylex32/athenastreamdownloader/blob/main/img/R.png?raw=true" Width="27" Margin="0.5,0,0,0"/>
    </StackPanel>
    </Button>
    </Grid>
</Window>
...
$jsonSettings = Get-Content -Path "settings.json" | ConvertFrom-Json
 ...
 $script:QMusicpath = $jsonSettings.Qobuz
 $script:Qobuzappid = $jsonSettings.Qapp_id
 $script:QobuzSecrets = $jsonSettings.Qsecrets
 $script:QobuzToken = $jsonSettings.Qtoken
 $script:SpotMusicpath = $jsonSettings.Spotify
 $script:SpotLayoutMusic = $jsonSettings.S_Track
 $script:SpotTracknumber = $jsonSettings.trackNumber
 $script:QQualityFiles = $jsonSettings.Q_Quality
 $script:QLayoutMusic = $jsonSettings.Q_Album
 $script:QTextFiles = $jsonSettings.QTxt
 $script:DMusicpath = $jsonSettings.Deezer
 $script:DQualityFiles = $jsonSettings.D_Quality
 $script:DLayoutMusic = $jsonSettings.D_Album
 $script:DTextFiles = $jsonSettings.DTxt
...
function SearchNewmusic {
    if ($FeaturedRadiobutton.IsChecked){
        ...
        $albumlist = Invoke-WebRequest -Uri "$BaseUri$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET -Headers $headers
    ...
    elseif ($Search_Box.text -eq 'Album of the week') 
    {
        ...
        $headers.Add("Content-Type", "application/json")
        $BaseUri = "https://www.qobuz.com/api.json/0.2/featured/index?offset=0&offset=0&limit=50&limit=50&extra=track&format_id="
        $albweeklist = Invoke-WebRequest -Uri "$BaseUri$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET -Headers $headers 
    ...
    elseif ($Search_Box.text -eq 'Rap') 
    {
        ...
        $RapGenreList = Invoke-WebRequest -Uri "$($BaseUri)$($GenreRap)format_id=$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET 
    ...
    elseif ($Search_Box.text -eq 'Pop') 
    {
        ...
        $BaseUri = "https://www.qobuz.com/api.json/0.2/featured/index?"
        $PopGenreList = Invoke-WebRequest -Uri "$($BaseUri)$($GenrePop)format_id=$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET 
    ...
    elseif ($Search_Box.text -eq 'Latin') 
    {
        ...
        $BaseUri = "https://www.qobuz.com/api.json/0.2/featured/index?"
        $LatinGenreList = Invoke-WebRequest -Uri "$($BaseUri)$($GenreLatin)format_id=$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET 
    ...
    elseif ($Search_Box.text -eq 'Electro') 
    {
        ...
        $BaseUri = "https://www.qobuz.com/api.json/0.2/featured/index?"
        $ElectroGenreList = Invoke-WebRequest -Uri "$($BaseUri)$($GenreElectro)format_id=$script:QobuzToken&app_id=$script:Qobuzappid" -UseBasicParsing -Method GET 
...
function SearchArtist  {
    ...
    if ($ArtistRadiobutton.IsChecked)
    {
        ...
        $urlapi = "$($BaseUri)$($searchout)&offset=0&offset=0&limit=25&limit=25&extra=albums&format_id=$($script:QobuzToken)&app_id=$($script:Qobuzappid)" 
    ...
    elseif ($SongsRadiobutton.IsChecked) 
    {
        ...
        $urltrackapi = "$($BaseUri)$($searchout)&offset=0&limit=25&extra=track&format_id=$($script:QobuzToken)&app_id=$($script:Qobuzappid)" 
    ...
    elseif ($PlaylistRadiobutton.IsChecked){
        ...
        $Albums_list = [System.Collections.Generic.List[PSCustomObject]]::new()
        $authorization=@{}
        $authorization.Add("X-User-Auth-Token", "$($script:QobuzToken)")
        $authorization.Add("X-App-Id", "$($script:Qobuzappid)")
...
function URL  {
    ...
    {   
        ...
        #Write-Host "`n"
        Write-Host 'Starting Conversion for Playlist Name:'"$PlaylistName"
        #Write-Host "`n"
        ...
        $headers=@{}
        $headers.Add("Content-Type", "application/json")
        $urltrackapi = "https://www.qobuz.com/api.json/0.2/catalog/search?query=$searchout&offset=0&limit=1&extra=track&format_id=$script:QobuzToken&app_id=$script:Qobuzappid" 
        ...
        $Spotjobs = ".\d-fi.exe /c -q '$script:QQualityFiles' -b -o '$script:SpotMusicpath/$PlaylistName/$script:SpotLayoutMusic' -d -i '$Playlistpath'; Get-ChildItem -Path '$script:SpotMusicpath/$PlaylistName' -Filter '*.flac' -ErrorAction SilentlyContinue -Recurse | Select-Object -ExpandProperty FullName | Sort-Object | Add-Content '$script:SpotMusicpath/$PlaylistName.m3u8' -Encoding utf8 -Force; Remove-Item '$Trackpath' -ErrorAction SilentlyContinue; Remove-Item '$Playlistpath' -ErrorAction SilentlyContinue; Exit"
         # Start a new PowerShell process using Start-Process
         Start-Process powershell.exe -ArgumentList "-NoExit", "-Command", $Spotjobs
        try {Invoke-WebRequest -Uri "$playlistcover" -OutFile "$script:SpotMusicpath/$PlaylistName.jpg" -ErrorAction SilentlyContinue | Out-Null} catch {continue}
        $Search_Box.Clear()
    ...
    elseif ($DeezerUrlRadiobutton.IsChecked)
    {
        ...
        {
         $deezerjobs = ".\d-fi.exe /c -u '$($Albumsindex.Where({$_.Albums -eq $Albums_Listbox.SelectedItem}).QobuzUrl)' -q '$script:DQualityFiles' -o '$script:DMusicpath/$script:DLayoutMusic' -d; Exit"
         # Start a new PowerShell process using Start-Process
         ...
         $PlaylistJobs = ".\d-fi.exe /c  -q '$script:DQualityFiles' -o '$script:DMusicpath/$script:DLayoutMusic' -d -i '$Playlistpath'; Exit"
         # Start a new PowerShell process using Start-Process
         ...
         # Construct the command string
         $Qobuzjobs = ".\d-fi.exe /c -u '$albumsresults' -q '$script:QQualityFiles' -d -b -o '$script:QMusicpath\$script:QLayoutMusic'; Exit"
...
$Status_Button.Add_Click({
    ...
    # Create a grid to hold the settings controls
    $grid = New-Object -TypeName System.Windows.Controls.Grid
    $window.Content = $grid
    
    # Define column definitions for the grid
    $columnDefinition = New-Object -TypeName System.Windows.Controls.ColumnDefinition
    ...
    # Define a row for the "Settings" label
    $rowDefinition = New-Object -TypeName System.Windows.Controls.RowDefinition
    $rowDefinition.Height = [System.Windows.GridLength]::new(30, [System.Windows.GridUnitType]::Pixel)
    $grid.RowDefinitions.Add($rowDefinition)
    
    # Create the "Settings" label
    $settingsLabel = New-Object -TypeName System.Windows.Controls.Label
    ...
    $settingsLabel.SetValue([System.Windows.Controls.Grid]::ColumnSpanProperty, 3)
    $settingsLabel.Margin = New-Object System.Windows.Thickness(0) # Add margin to create spaces around the label
    $grid.Children.Add($settingsLabel)
    
    # Continue with the other rows
    $row = 1
    $labels = "Qobuz", "Deezer", "Spotify", "Q_Quality", "Q_Album", "Qapp_id", "Qsecrets", "Qtoken", "S_Track", "D_Quality", "D_Album"
    foreach ($label in $labels) {
        $rowDefinition = New-Object -TypeName System.Windows.Controls.RowDefinition
        $rowDefinition.Height = [System.Windows.GridLength]::new(30, [System.Windows.GridUnitType]::Pixel)
        $grid.RowDefinitions.Add($rowDefinition)
    
        $labelControl = New-Object -TypeName System.Windows.Controls.Label
        $labelControl.Content = $label
        $labelControl.HorizontalAlignment = "Center"
        $labelControl.Margin = New-Object System.Windows.Thickness(0,5,10,0) # Add margin to create spaces around the label
        $labelControl.SetValue([System.Windows.Controls.Grid]::ColumnProperty, 0)
        $labelControl.SetValue([System.Windows.Controls.Grid]::RowProperty, $row)
        $grid.Children.Add($labelControl)
    
        $textBox = New-Object -TypeName System.Windows.Controls.TextBox
        $textBox.Text = $jsonSettings.$label
        $textBox.TextAlignment = "Center"
        $textBox.Margin = New-Object System.Windows.Thickness(0,10,10,0) # Add margin to create spaces around the textbox
        $textBox.Background = New-Object System.Windows.Media.SolidColorBrush ([System.Windows.Media.Colors]::DarkGray) # Set the background color
        $textBox.SetValue([System.Windows.Controls.Grid]::ColumnProperty, 1)
        $textBox.SetValue([System.Windows.Controls.Grid]::RowProperty, $row)
        $grid.Children.Add($textBox)
    
        if ($label -in "Qobuz", "Spotify", "Deezer") {
            $selectPathButton = New-Object -TypeName System.Windows.Controls.Button
            $selectPathButton.Content = "Browse"
            $selectPathButton.Width = 50
            $selectPathButton.Height = 20
            $selectPathButton.Margin = New-Object System.Windows.Thickness(0,10,10,0) # Add margin to create spaces around the pathbutton
            $selectPathButton.Add_Click({
                $folderBrowser = New-Object -TypeName System.Windows.Forms.FolderBrowserDialog
                $result = $folderBrowser.ShowDialog()
                if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
                    $button = $this
                    $textBox = $grid.Children | Where-Object { $_.GetValue([System.Windows.Controls.Grid]::RowProperty) -eq $button.GetValue([System.Windows.Controls.Grid]::RowProperty) -and $_.GetValue([System.Windows.Controls.Grid]::ColumnProperty) -eq 1 }
                    $textBox.Text = $folderBrowser.SelectedPath
                }
            })
            $selectPathButton.SetValue([System.Windows.Controls.Grid]::ColumnProperty, 2)
            $selectPathButton.SetValue([System.Windows.Controls.Grid]::RowProperty, $row)
            $grid.Children.Add($selectPathButton)
        }
    
        $row++
    }
    
     # Continue after the last row increment
     $row++

     # Add an empty row for spacing
     $rowDefinition = New-Object -TypeName System.Windows.Controls.RowDefinition
     $rowDefinition.Height = [System.Windows.GridLength]::new(10, [System.Windows.GridUnitType]::Pixel) # Set the height to adjust the space
     $grid.RowDefinitions.Add($rowDefinition)
     $row++

     # Define new row for Save button
     $rowDefinition = New-Object -TypeName System.Windows.Controls.RowDefinition
     $rowDefinition.Height = [System.Windows.GridLength]::new(30, [System.Windows.GridUnitType]::Pixel)
     $grid.RowDefinitions.Add($rowDefinition)

     # Create a Save button
     $saveButton = New-Object -TypeName System.Windows.Controls.Button
     $saveButton.Content = "Save"
     $saveButton.Width = 100  # Set the width of the button
     $saveButton.SetValue([System.Windows.Controls.Grid]::ColumnSpanProperty, 3)
     $saveButton.SetValue([System.Windows.Controls.Grid]::ColumnProperty, 0)
     $saveButton.SetValue([System.Windows.Controls.Grid]::RowProperty, $row)
     $grid.Children.Add($saveButton)

     $saveButton.Add_Click({
        $counter = 1
        foreach ($label in $labels) {
            $textBox = $grid.Children | Where-Object { $_.GetValue([System.Windows.Controls.Grid]::RowProperty) -eq $counter -and $_.GetValue([System.Windows.Controls.Grid]::ColumnProperty) -eq 1 } | ForEach-Object { $_ }
            $text = $textBox.Text -replace '\\', '/'
            $text = $text -replace '^\"|\"$', ''
            $jsonSettings.$label = $text
            $counter++
        }
    
        $jsonSettings | ConvertTo-Json | Set-Content -Path "settings.json"
        $script:QMusicpath = $jsonSettings.Qobuz
        $script:Qobuzappid = $jsonSettings.Qapp_id
        $script:QobuzSecrets = $jsonSettings.Qsecrets
        $script:QobuzToken = $jsonSettings.Qtoken
